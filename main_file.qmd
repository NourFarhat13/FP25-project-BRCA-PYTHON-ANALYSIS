...
brca.isnull().sum().pipe(lambda s: s[s > 0])
...
for ax, col, title in [
    (axes[0], "cluster", "K-Means Clusters"),
    (axes[1], "patient_status", "Actual Survival Status"),
]:
    for val in sorted(brca_scaled[col].unique()):
        mask = brca_scaled[col] == val
        label = f"Cluster {val}" if col == "cluster" else val
        ax.scatter(
            brca_scaled.loc[mask, "protein1"],
            brca_scaled.loc[mask, "protein2"],
            label=label, alpha=0.6, s=40,
        )
    ax.set_xlabel("Protein1 (scaled)")
    ax.set_ylabel("Protein2 (scaled)")
    ax.set_title(title)
    ax.legend()
...
print((pd.crosstab(brca_scaled["cluster"], brca_scaled["patient_status"], normalize="index") * 100).round(1))
```
